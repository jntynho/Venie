...
export const TagLinks: React.FC = () => {
  ...
  const associatedLinks = useMemo(() => {
    ...
    return state.links.filter(l => !l.isDeleted && l.tagIds.includes(id));
  ...
  const tag = useMemo(() => tagMap.get(id || ''), [tagMap, id]);
  ...
  if (!tag) return <Navigate to="/manage-tags" replace />;
    ...
    <div className="flex flex-col gpu-accelerated animate-slide-in" style={{ backgroundColor: 'var(--bg)' }}>
      <div className="px-5 mb-0 mt-0" style={{ backgroundColor: 'var(--bg)' }}>
         <div className="flex items-center gap-7 py-10 border-b border-[var(--border)]">
            <div 
              className="w-24 h-24 rounded-full flex items-center justify-center bg-[var(--surface)] border-[4px] shrink-0 text-yellow-400 overflow-hidden shadow-2xl ring-1 ring-black/10 transition-colors duration-500"
